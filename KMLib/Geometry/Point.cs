namespace KMLib.Geometry
{
    ...
    public class KmlPoint : AGeometry
    {
        ...
        public string coordinates
        {
            ...
            set
            {
                var bits = StringUtils.Split(value, ",");
                if (bits.Length < 2)
                {
                    Fail(value);
                }

                if (!float.TryParse(bits[0], out m_Longitude))
                {
                    Fail(value);
                }

                if (!float.TryParse(bits[1], out m_Latitude))
                {
                    Fail(value);
                }
                ...
                if (bits.Length == 3)
                {
                    if (!float.TryParse(bits[2], out m_Altitude))
                    {
                        Fail(value);
                    }
                }
