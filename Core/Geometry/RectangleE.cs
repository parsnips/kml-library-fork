namespace Core.Geometry
{
    ...
    public class RectangleE : AShapeE, ICloneable, IComparable<RectangleE>
    {
        ...
        public SizeE Size
        {
            get { return new SizeE(Width, Height); }
            set
            {
                Width = value.Width;
                Height = value.Height;
        ...
        {
            get { return new PointE(X + Width/2, Y + Height/2); }
            set
            {
                X = value.X - Width/2;
                Y = value.Y - Height/2;
        ...
        {
            get { return new PointE(X, Y + Height); }
            set
            {
                X = value.X;
                Y = value.Y - Height;
        ...
        {
            get { return new PointE(X + Width, Y + Height); }
            set
            {
                X = value.X - Width;
                Y = value.Y - Height;
        ...
        {
            get { return new PointE(X + Width, Y); }
            set
            {
                X = value.X - Width;
                Y = value.Y;
        ...
        {
            get { return new IntervalE(X, Right); }
        }

        [XmlIgnore]
        public IntervalE YInterval
        {
            get { return new IntervalE(Y, Top); }
        }
        ...
        public RectangleE Union(PointE pt)
        {
            ...
            var xInt = XInterval.Union(pt.X);
            var ans = new RectangleE(xInt, YInterval.Union(pt.Y));
            return ans;
        }
        ...
        public override bool MostlyContains(IShapeE shapeE, double tolerance)
        {
            ...
            var intersect = Intersect(shapeE.BoundingBox);
        ...
        {
            var xIntersect = XInterval.Intersect(r.XInterval);
            var yIntersect = YInterval.Intersect(r.YInterval);
...
// RectangleC
        ...
        public static explicit operator Rectangle(RectangleE r)
        {
            return new Rectangle((int) r.X, (int) r.Y, (int) r.Width, (int) r.Height);
        }

        public static explicit operator RectangleF(RectangleE r)
        {
